namespace Rocket.Chat.Net.Driver
{
    ...
    public class DdpClient : IDdpClient
    {
        ...
        {
            _logger.LogInformation("ERROR: " + errorEventArgs?.Exception?.Message);
        }

        private void SocketOnOpened(object sender, EventArgs eventArgs)
        {
            _logger.LogDebug("OPEN");

            _logger.LogDebug("Sending connection request");
        ...
        private void SocketOnMessage(object sender, PortableMessageReceivedEventArgs messageEventArgs)
        {
            ...
            _logger.LogDebug($"RECIEVED: {JsonConvert.SerializeObject(data, Formatting.Indented)}");
        ...
        private void InternalHandle(string type, JObject data)
        {
            ...
            switch (type)
            {
                ...
                case "connected":
                    ...
                    _logger.LogDebug($"Connected via session {SessionId}.");
                    break;
                case "ready":
                    var subs = data["subs"];
